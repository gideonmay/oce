...
Handle(WfObjMesh_Mesh) WfObjMesh::Merge(const Handle(WfObjMesh_Mesh) &mesh1, const Handle(WfObjMesh_Mesh) &mesh2) {
    Handle(WfObjMesh_Mesh) mergeMesh = new WfObjMesh_Mesh;
    WfObjMesh_SequenceOfMeshTriangle aSeqOfTriangle;
    ...
    for (idom = 1; idom <= mesh1->NbDomains(); idom++) {
        ...
        for (Standard_Integer itri = 1; itri <= mesh1->NbTriangles(idom); itri++) {
            const Handle(WfObjMesh_MeshTriangle) aTrian = aSeqOfTriangle.Value(itri);
