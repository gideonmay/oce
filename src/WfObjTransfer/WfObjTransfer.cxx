...
void WfObjTransfer::BuildIncrementalMesh (const TopoDS_Shape&  Shape,
										const Standard_Real  Deflection,
										const Standard_Boolean  InParallel,
										const Handle(WfObjMesh_Mesh)& Mesh)
{
  if (Deflection <= Precision::Confusion ()) {
    Standard_ConstructionError::Raise ("WfObjTransfer::BuildIncrementalMesh");
    }
  ...
  for (TopExp_Explorer itf(Shape,TopAbs_FACE); itf.More(); itf.Next()) {
	...
	Mesh->AddTriangle (A, B, C, average.X(), average.Y(), average.Z());
      ...
      {
#ifdef OCCT_DEBUG
	cout << "Fail in WfObjTransfer::BuildIncrementalMesh"   << endl;
#endif
      }
  }
}
